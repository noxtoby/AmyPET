...
def r_trimup(fpet, fmri, outpath=None, store_img_intrmd=True):
    ...
    return dict(trmdir=trmdir, ftrm=ftrm, trim_scale=scale) #return dict(trmdir=trmdir, ftrm=ftrm['fimi'][0], trim_scale=scale)
# ========================================================================================
...
def voi_process(
    ...
    if save_voi_masks:
        ...
        mask_dir = None
    voival = extract_vois(out['ftrm']['im'], plbl_dct, voi_dct, outpath=mask_dir, output_masks=output_masks)
    ...
    if qc_plot and output_masks:
        showpet = nimpa.imsmooth(out['ftrm']['im'].astype(np.float32), voxsize=plbl_dct['voxsize'], fwhm=3.)
        ...
        ax[0,1].text(0, out['ftrm']['im'].shape[1]+10, suvrtxt, fontsize=12)
